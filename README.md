...
Without a member class or services, the container still compiles: stand-ins throw as soon as
something asks for a member, naming the key that is missing.
...
| `campaigns:migrate:interests` | adopts the provider's interest IDs and the membership behind them |
...
Coming from Pimcore's Customer Management Framework, two steps come first, in order: pull the
statuses, then import the provider IDs. Skipping either costs you a mass unsubscribe or a duplicate
of every group, segment and template. → **[docs/migrating-from-cmf.md](docs/migrating-from-cmf.md)**
