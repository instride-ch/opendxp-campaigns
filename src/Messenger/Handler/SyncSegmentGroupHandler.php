...
#[AsMessageHandler]
readonly class SyncSegmentGroupHandler
{
    ...
    public function __invoke(SyncSegmentGroupMessage $message): void
    {
        $object = DataObject::getById($message->objectId);

        if (!$object instanceof NewsletterSegmentGroupInterface) {
            $this->logger->warning(
                '[OpenDXP Campaigns] Segment group {id} not found or not a NewsletterSegmentGroup, skipping.',
                ['id' => $message->objectId],
            );
        ...
        $this->exporter->exportGroup($object);
    }
}